...
from app.modules.patients.schemas import PatientCreate, PatientRead
...
async def create_patient(payload: PatientCreate, user: Staff, session: Db) -> Patient:
    ...
    record_event(session, actor_id=user.id, action="patient.created", resource_type="patient", resource_id=patient.id)
...
async def get_patient(patient_id: UUID, user: Staff, session: Db) -> Patient:
    patient = await session.get(Patient, patient_id)
    if patient is None:
        raise NotFoundError("Patient not found")
    record_event(session, actor_id=user.id, action="patient.viewed", resource_type="patient", resource_id=patient.id)
    await session.commit()
    return patient
